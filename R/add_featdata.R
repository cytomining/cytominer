...
add_featdata.profile.data <- function(P, P_new, ...) {
  ...
  testthat::expect_true(all(names(meta(P_new)) %in% names(meta(P))),
                        info = stringr::str_c(setdiff(names(meta(P_new)),
                        names(meta(P))), collapse = ","))
  ...
  futile.logger::flog.debug("These cols will be used to join the metadata: %s",
                            stringr::str_c(names(meta(P_new)), collapse = ","))
  meta_old <- P$metadata
  # Get full data of the new profile.data
  full_new <- full(P_new, keep_xid = F)
  testthat::expect_equal(intersect(names(meta_old),
                                   names(full_new)) %>% sort(),
                         intersect(names(meta_old),
                                   names(meta(P_new))) %>% sort()
                        )
