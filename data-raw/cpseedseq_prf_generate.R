...
cpseedseq_prf <- cytominr:::profile.data(cfg_fname)

cpseedseq_prf <- cytominr:::process_metadata(cpseedseq_prf,
  strip_cellprofiler_db_tags = T)

cpseedseq_prf$metadata <- dplyr::filter(cpseedseq_prf$metadata,
                                        Plate %in% c(38034, 38003, 37983))

cpseedseq_prf <- cytominr:::post_filter_metadata(cpseedseq_prf)
