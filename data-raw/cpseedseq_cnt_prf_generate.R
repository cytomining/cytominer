...
cpseedseq_cnt_prf <- cytominr:::profile.data(cfg_fname)
cpseedseq_cnt_prf %<>% cytominr:::process_metadata.profile.data(strip_cellprofiler_db_tags = T)
cpseedseq_cnt_prf$metadata %<>% dplyr::filter(Plate %in% c(38034, 38003, 37983))
cpseedseq_cnt_prf %<>% cytominr:::post_filter_metadata.profile.data()
